...
function hourKey(dateValue) {
  ...
  const get = function(type) {
    const item = parts.find(function(p) {
      return p.type === type;
    });
...
export default async function handler(req, res) {
  try {
    ...
    const response = await fetch(cwaRequestUrl, {
      cache: "no-store"
    });

    if (!response.ok) {
      throw new Error(
        "CWA HTTP " + response.status
      );
    }
    ...
    const station =
      stations.find(function(item) {
        ...
        return String(id).toUpperCase() === STATION_ID;
      }) ||
